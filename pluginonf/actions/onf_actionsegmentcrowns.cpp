...
ONF_ActionSegmentCrowns::UndoRedoContent::UndoRedoContent()
{
    _useNewClustersMap = true;
}

ONF_ActionSegmentCrowns::UndoRedoContent::UndoRedoContent(const int newCluster, const QMap<size_t, int> &indices)
{
...
ONF_ActionSegmentCrowns::UndoRedoContent::UndoRedoContent(const int newCluster)
{
    _newCluster = newCluster;
    _useNewClustersMap = false;
}

void ONF_ActionSegmentCrowns::UndoRedoContent::addIndice(const size_t indice, const int oldCluster)
{
    _oldClusters.insert(indice, oldCluster);
    _useNewClustersMap = false;
}

void ONF_ActionSegmentCrowns::UndoRedoContent::addIndice(const size_t indice, const int oldCluster, const int newCluster)
{
    _oldClusters.insert(indice, oldCluster);
...
ONF_ActionSegmentCrowns::ONF_ActionSegmentCrowns(const CT_Grid2DXY<int> *densityGrid, const CT_Grid2DXY<double> *mnsGrid, CT_Grid2DXY<int> *clustersGrid) : CT_AbstractActionForGraphicsView()
{
...
ONF_ActionSegmentCrowns::~ONF_ActionSegmentCrowns()
{
    delete _whiteColor;
...
QString ONF_ActionSegmentCrowns::uniqueName() const
{
    return "ONF_ActionSegmentCrowns";
}

QString ONF_ActionSegmentCrowns::title() const
{
    return "Segment crowns";
}

QString ONF_ActionSegmentCrowns::description() const
{
    return "Segmente les houppiers";
}

QIcon ONF_ActionSegmentCrowns::icon() const
{
    return QIcon(":/icons/limits.png");
}

QString ONF_ActionSegmentCrowns::type() const
{
    return CT_AbstractAction::TYPE_MODIFICATION;
}

const QColor* ONF_ActionSegmentCrowns::getNextColor()
{
...
void ONF_ActionSegmentCrowns::init()
{
    ...
    if(nOptions() == 0)
    {
        ...
        ONF_ActionSegmentCrownsOptions *option = new ONF_ActionSegmentCrownsOptions(this, _mnsGrid->level());

        option->setMode(ONF_ActionSegmentCrownsOptions::FREEMOVE);
        option->setDrawingGrid(ONF_ActionSegmentCrownsOptions::CLUSTERS);
...
void ONF_ActionSegmentCrowns::redrawOverlay()
{
    document()->redrawGraphics();
}

void ONF_ActionSegmentCrowns::redrawOverlayAnd3D()
{
    setDrawing3DChanged();
}

void ONF_ActionSegmentCrowns::initClusters()
{
    PS_LOG->addMessage(LogInterface::info, LogInterface::action, QString("ONF_ActionSegmentCrowns : Initialisation des clusters"));

    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
void ONF_ActionSegmentCrowns::drawLimit(ONF_ActionSegmentCrownsOptions *option, size_t maxCol, size_t maxRow)
{
...
void ONF_ActionSegmentCrowns::fillCellsInList(QList<size_t> &liste, const int cluster, bool record)
{
...
QList<size_t> ONF_ActionSegmentCrowns::computeColonize(size_t originColumn, size_t originRow)
{
    QList<size_t> result;
    size_t index;
...
void ONF_ActionSegmentCrowns::appendIfNotNulValue(QList<size_t> &result, size_t col, size_t lin)
{
    size_t index;
...
void ONF_ActionSegmentCrowns::fillCluter(const size_t col, const size_t row, const int fillingCluster)
{
    QList<size_t> result;
    size_t index;
...
void ONF_ActionSegmentCrowns::fillAllCluterPixels(const size_t col, const size_t row, const int fillingCluster)
{
    size_t index;
    if (!_clustersGrid->index(col, row, index)) {return;}
...
void ONF_ActionSegmentCrowns::appendIfSameCluster(QList<size_t> &result, size_t col, size_t lin, int cluster)
{
    size_t index;
    if (_clustersGrid->index(col, lin, index))
    {
...
void ONF_ActionSegmentCrowns::addCluster(bool updateUi)
{
...
void ONF_ActionSegmentCrowns::changeActiveClusterColor()
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
void ONF_ActionSegmentCrowns::mergeClusters(int pixelNumber)
{
...
void ONF_ActionSegmentCrowns::addUndoContent(UndoRedoContent* content)
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
void ONF_ActionSegmentCrowns::undo()
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
void ONF_ActionSegmentCrowns::redo()
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
bool ONF_ActionSegmentCrowns::getCoordsForMousePosition(const QMouseEvent *e, double &x, double &y)
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
bool ONF_ActionSegmentCrowns::mousePressEvent(QMouseEvent *e)
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
    ...
    if ((e->buttons() & Qt::LeftButton))
    {
        ...
        if (getCoordsForMousePosition(e, x, y))
        {
            ...
            if (option->getMode() == ONF_ActionSegmentCrownsOptions::CHOOSECLUSTER)
            {
                ...
                redrawOverlayAnd3D();
                return true;
            } else if (option->getMode() == ONF_ActionSegmentCrownsOptions::CHANGECENTERCELL)
            {
            ...
            } else
            {
                ...
                if (option->getMode() == ONF_ActionSegmentCrownsOptions::DRAWLIMITS)
                {
                    drawLimit(option, maxCol, maxRow);
                    redrawOverlayAnd3D();
                    return true;
                } else if (option->getMode() == ONF_ActionSegmentCrownsOptions::FILLAREA)
                {
                    for (size_t c = _activeCol ; c <= maxCol ; c++)
                    {
                        for (size_t r = _activeRow ; r <= maxRow ; r++)
...
bool ONF_ActionSegmentCrowns::mouseMoveEvent(QMouseEvent *e)
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
bool ONF_ActionSegmentCrowns::mouseReleaseEvent(QMouseEvent *e)
{
    Q_UNUSED(e);
    return false;
}

bool ONF_ActionSegmentCrowns::wheelEvent(QWheelEvent *e)
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);
...
bool ONF_ActionSegmentCrowns::keyPressEvent(QKeyEvent *e)
{
    ONF_ActionSegmentCrownsOptions *option = (ONF_ActionSegmentCrownsOptions*)optionAt(0);

    if((e->key() == Qt::Key_S) && !e->isAutoRepeat())
    {
        option->setMode(ONF_ActionSegmentCrownsOptions::CHOOSECLUSTER);
        return true;
    } else if((e->key() == Qt::Key_D) && !e->isAutoRepeat())
    {
        option->setMode(ONF_ActionSegmentCrownsOptions::DRAWLIMITS);
        return true;
    } else if((e->key() == Qt::Key_F) && !e->isAutoRepeat())
    {
        option->setMode(ONF_ActionSegmentCrownsOptions::FILLAREA);
        return true;
...
bool ONF_ActionSegmentCrowns::keyReleaseEvent(QKeyEvent *e)
{
    Q_UNUSED(e);
    return false;
}

void ONF_ActionSegmentCrowns::draw(GraphicsViewInterface &view, PainterInterface &painter)
{
    Q_UNUSED(view)
    ...
    if (option->getDrawingGrid() == ONF_ActionSegmentCrownsOptions::DENSITY)
    {
    ...
    for (size_t cx = 0 ; cx < _clustersGrid->colDim() ; cx++)
    {
        for (size_t ly = 0 ; ly < _clustersGrid->linDim() ; ly++)
        {
            ...
            if (option->getDrawingGrid() == ONF_ActionSegmentCrownsOptions::CLUSTERS)
            {
    ...
    if (option->getMode() == ONF_ActionSegmentCrownsOptions::DRAWLIMITS ||
            option->getMode() == ONF_ActionSegmentCrownsOptions::FILLAREA)
    {
        painter.setColor(QColor(255, 0, 0));
...
void ONF_ActionSegmentCrowns::drawPencil(PainterInterface &painter, ONF_ActionSegmentCrownsOptions *option, const double &resolution, const double &z_val)
{
    int size = option->getPencilSize();
...
void ONF_ActionSegmentCrowns::drawOverlay(GraphicsViewInterface &view, QPainter &painter)
{
    Q_UNUSED(view);
...
CT_AbstractAction *ONF_ActionSegmentCrowns::copy() const
{
    return new ONF_ActionSegmentCrowns(_densityGrid, _mnsGrid, _clustersGrid);
}
