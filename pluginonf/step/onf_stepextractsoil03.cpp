...
#include "onf_stepextractsoil03.h"
...
ONF_StepExtractSoil03::ONF_StepExtractSoil03(CT_StepInitializeData &dataInit) : CT_AbstractStep(dataInit)
{
    _gridsize   = 0.5;
    _soilwidth = 0.32;
    _min_density = 200;
...
QString ONF_StepExtractSoil03::getStepDescription() const
{
    return tr("Séparation sol / végétation -> MNT");
}

QString ONF_StepExtractSoil03::getStepDetailledDescription() const
{
...
CT_VirtualAbstractStep* ONF_StepExtractSoil03::createNewInstance(CT_StepInitializeData &dataInit)
{
    // cree une copie de cette etape
    return new ONF_StepExtractSoil03(dataInit);
}

/////////////////////// PROTECTED ///////////////////////

void ONF_StepExtractSoil03::createInResultModelListProtected()
{  
...
void ONF_StepExtractSoil03::createPostConfigurationDialog()
{
...
void ONF_StepExtractSoil03::createOutResultModelListProtected()
{
    ...
    resultModel->addItemModel(DEF_SearchOutGroupMNT, DEF_SearchOutMNT, new CT_Grid2DXY<double>(), tr("MNT (Raster)"));
    ...
    resultModel->addItemModel(DEF_SearchOutGroupMNS, DEF_SearchOutMNS, new CT_Grid2DXY<double>(), tr("MNS (Raster)"));
    ...
    resultModel->addItemModel(DEF_SearchOutGroupMNH, DEF_SearchOutMNH, new CT_Grid2DXY<double>(), tr("MNH (Raster)"));
    ...
    resultModel->addItemModel(DEF_SearchOutGroupDensite, DEF_SearchOutDensite, new CT_Grid2DXY<int>(), tr("Densité pts sol (Raster)"));
}

void ONF_StepExtractSoil03::compute()
{
    ...
    if (!isStopped() && it.hasNext())
    {
        ...
        CT_Grid2DXY<double>* mnt = new CT_Grid2DXY<double>(DEF_SearchOutMNT, outResultMNT, minX, minY, n_mntX, n_mntY, _gridsize, minZ, -9999, std::numeric_limits<double>::max());
        CT_Grid2DXY<double>* mns = new CT_Grid2DXY<double>(DEF_SearchOutMNS, outResultMNS, minX, minY, n_mntX, n_mntY, _gridsize, maxZ, -9999, -std::numeric_limits<double>::max());
        CT_Grid2DXY<double>* mnh = new CT_Grid2DXY<double>(DEF_SearchOutMNH, outResultMNH, minX, minY, n_mntX, n_mntY, _gridsize, maxZ + 1, -9999, std::numeric_limits<double>::max());
        CT_Grid2DXY<int>* densite = new CT_Grid2DXY<int>(DEF_SearchOutDensite, outResultDensite, minX, minY, n_mntX, n_mntY, _gridsize, minZ - 1, -9999, 0);
        ...
        while (!isStopped() && it2.hasNext())
        {
            ...
            while(itP.hasNext() && !isStopped())
            {
                ...
                mnt->setMinValueAtXY(point(0), point(1), point(2));
                mns->setMaxValueAtXY(point(0), point(1), point(2));
            }
        }

        // MNS : cases sans points => NA
        size_t xx = 0;
        size_t yy = 0;
        for (xx=0 ; xx<n_mntX ; ++xx) {
            for (yy=0 ; yy<n_mntY ; ++yy) {

                double valueMNS = mns->value(xx,yy);

                if (valueMNS == -std::numeric_limits<double>::max()) {
                    mns->setValue(xx, yy, mnh->NA());
        ...
        while (!isStopped() && it3.hasNext())
        {
            ...
            {
                const CT_Point &point =itP.next().currentPoint();
                double value = mnt->valueAtXY(point(0), point(1)) + _soilwidth;
                if (point(2) < value)
                {
                    densite->addValueAtXY(point(0), point(1), 1);
                }
            ...
            for (yy=0 ; yy<n_mntY ; ++yy) {
                ...
                if (value != mnt->NA())
                {
                    if (densite->value(xx,yy) < min_density)
                    {
                        ...
                        QList<double> neighbours = mnt->neighboursValues(xx, yy, _dist, false, CT_Grid2DXY<double>::CM_DropCenter);
                        ...
                        if (size_neighbours > 0) {
                            ...
                            double median = neighbours.at(med_ind);
                            double val_test = std::min(fabs(neighbours.last() - median), fabs(neighbours.first()  - median));
        ...
        if (_interpol) {
            CT_Grid2DXY<double>* MNTinterpol = (CT_Grid2DXY<double>*) mnt->copy(DEF_SearchOutMNT, outResultMNT, CT_ResultCopyModeList() << CT_ResultCopyModeList::CopyItemDrawableReference);
            for (xx=0 ; xx<n_mntX ; ++xx) {
                for (yy=0 ; yy<n_mntY ; ++yy) {

                    double value = mnt->value(xx,yy);
                    double x = mnt->getCellCenterColCoord(xx);
                    double y = mnt->getCellCenterLinCoord(yy);
        ...
        if (_smooth) {
            CT_Grid2DXY<double>* MNTsmoothed = (CT_Grid2DXY<double>*) mnt->copy(DEF_SearchOutMNT, outResultMNT, CT_ResultCopyModeList() << CT_ResultCopyModeList::CopyItemDrawableReference);
            for (xx=0 ; xx<n_mntX ; ++xx) {
                for (yy=0 ; yy<n_mntY ; ++yy) {

                    QList<double> neighbours = mnt->neighboursValues(xx, yy, _smoothDist, false, CT_Grid2DXY<double>::CM_DropCenter);
        ...
        for (xx=0 ; xx<n_mntX ; ++xx) {
            for (yy=0 ; yy<n_mntY ; ++yy) {
                ...
                double valueMNT = mnt->value(xx,yy);
                double valueMNS = mns->value(xx,yy);
                ...
                if ((valueMNT == mnt->NA()) || (valueMNS == mns->NA())) {
                    ...
                    double valueMNH = valueMNS - valueMNT;
